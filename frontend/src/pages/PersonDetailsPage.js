...
const PersonDetailsPage = (props) => {
    ...
    useEffect(() => {
        ...
        fetch(`https://api.themoviedb.org/3/person/${personId}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`)
            .then(res => res.json())
            .then(res => {
                console.log(res)
                setPerson(res)

                fetch(`https://api.themoviedb.org/3/person/${personId}/movie_credits?api_key=${process.env.REACT_APP_API_KEY}`)
                    .then(res => res.json())
                    .then(res => {
                        console.log(res)
                        setCredits(res.cast)
    ...
    return (
        <div>
            ...
            /> 
             ...
             {/* <div >
                    {Credits && Credits.map((credits, index) => (
                        
                            <FilmTable 
                                creditsTitle={credits.original_title}
                                creditsDate={credits.release_date}
                                creditsCharacter={credits.character}
                            />
    ...
    <div className="wrapper"> 
      ...
      <Table className={classes.table} aria-label="customized table" >
        <TableHead>
          <TableRow >
            <StyledTableCell><b>Movie</b></StyledTableCell>
            <StyledTableCell align="left" ><b>Character </b></StyledTableCell>
